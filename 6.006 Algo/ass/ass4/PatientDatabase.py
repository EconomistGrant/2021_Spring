...
class PatientDatabase(AVL):
    def __init__(self, item = None, parent = None):
        ...
        super().__init__(item, parent)
        ########################################################
        # TODO: initialize your augmentations #
        self.num_l = 0
        self.num_r = 0
        ########################################################
    ...
    def insert(self,name,priority):
        "insert into self's subtree"
        if self.is_empty():
            self.item = Patient(name,priority)
            self.maintain()# TODO: examine
        elif priority < self.item.key:
            if self.left is None:
                self.left  = self.__class__(None, self)
            self.num_l = self.num_l+1
            self.left.insert(name,priority)
        else:
            if self.right is None:
                self.right = self.__class__(None, self)
            self.num_r = self.num_r+1
            self.right.insert(name,priority)
    
    def find_1(self, priority):
        "return 1 if you can find a patient with this priority in the subtree of self"
        if self.is_empty():
            return 0
        elif priority == self.item.key:
            return 1
            ...
            if self.left == None:
                return 0
            return self.left.find_1(priority)
        else:
            if self.right == None:
                return 0
            return self.right.find_1(priority)
    ...
    def delete(self,priority):
        "delete a patient with this priority in the subtree of self"
        if self.find_1(priority) == 0: 
            raise IndexError('key not found in data structure')  
        else:
            return self.true_delete(priority)
        
    def true_delete(self,priority):
        "delete a patient with this priority in the subtree of self, and confident that the value exist"
        if priority == self.item.key:
            item = self.item
            self.remove()
            return item
        elif priority > self.item.key:
            self.num_r -= 1
            self.right.true_delete(priority)
        else:
            self.num_l -= 1
            self.left.true_delete(priority)
    ...
    def rotate_right(self):
        ...
        if a:   a.parent = self;num_a = a.num_l + a.num_r + 1 
        if b:                   num_b = b.num_l + b.num_r + 1
        if c:   c.parent = node;num_c = c.num_l + c.num_r + 1
        ...
        self.num_l = num_a
        self.num_r = num_b + num_c + 1
        self.right.num_l = num_b
        self.right.num_r = num_c
    ...
    def rotate_left(self):
        ...
        self.item, node.item = node.item, self.item
        num_a,num_b,num_c = 0,0,0
        if a:   a.parent = node;num_a = a.num_l + a.num_r + 1
        if b:                   num_b = b.num_l + b.num_r + 1
        if c:   c.parent = self;num_c = c.num_l + c.num_r + 1
        self.left, self.right = node, c
        node.left, node.right = a, b
        node.update()
        self.update()
        
        self.num_l = num_a + num_b + 1
        self.num_r = num_c
        
        self.left.num_l = num_a
        self.left.num_r = num_b


    
    def num_smaller(self,p):
        "return the number of patients strictly smaller than this priority"
        node = self
        num = 0
        while node:
            if    p < node.item.key: node = node.left
            elif  p > node.item.key: num+=node.num_l + 1;node = node.right
            else:                    num+=node.num_l;return num
        return num
        
    def num_patients(self,p1,p2):
        "return number of patients that have priority between p1 and p2"
        ########################################################
        # TODO: implement the num_patients function here #
        ########################################################
        
        return self.num_smaller(p2) - self.num_smaller(p1) + self.find_1(p2)
    ...
    def update_patient(self, name,priority):
        ...
        self.insert(name,priority)         # insert patient
...
if __name__ == '__main__':
    ...
    pd.update_patient("",31)
    pd.update_patient("",25)
    pd.update_patient("",33.231)
    pd.update_patient("",38)
    pd.update_patient("",50)
    pd.update_patient("",40)
    pd.update_patient("",46)
    pd.update_patient("",9)
    pd.update_patient("",45)
    pd.update_patient("",31)
    print(pd)
