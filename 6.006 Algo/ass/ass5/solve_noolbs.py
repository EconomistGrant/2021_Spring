...
def solve_noolbs(B):
    ...
    monkey_pos = find_monkey_position(B)
    balloons = num_balloons(B)
    #initialize
    
    for move in moves:
        node =  make_move(B,monkey_pos,balloons,3,move)
        if node is not None:
            if node[2] == 0: 
                return depth
            else:
    ...
    print(len(B))
    while len(to_visit) > 0 and depth <= (3*len(B)-3):
        depth = depth+1
        print("depth: " + str(depth) + "| num to visit" + str(len(to_visit)))
        new_to_visit, done, visited = find_adjacent(to_visit, visited)
        if done: return depth
        to_visit = new_to_visit
    
    return None

def find_adjacent(nodes, visited):
    #node = (B,monkey_pos,ballons,lives)
    moves = ["left","right","shoot"]
    adjacent_nodes = []
    done = False
    ...
    for node in nodes:
        for move in moves:
            adjacent_node = make_move(node[0],node[1],node[2],node[3],move)
            if (adjacent_node is not None) and (adjacent_node[3] > 0) and visited.get(adjacent_node, 0) == 0:
                adjacent_nodes.append(adjacent_node)
                visited[adjacent_node] = 1
                if adjacent_node[2] == 0: 
                    return adjacent_nodes, True, visited
            
    return adjacent_nodes, done, visited
